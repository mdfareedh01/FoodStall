...
import { LucideAngularModule, ArrowLeft, Save, X, Utensils, Leaf, Star, Tag, Info, Image as ImageIcon, Zap, Sprout, Home } from 'lucide-angular';
      ...
      <div class="container py-4 sm:py-6 px-4 max-w-5xl">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
               <button (click)="goBack()" hlmBtn variant="ghost" class="rounded-lg font-bold px-4 h-9 text-xs border-none hover:bg-muted/50">
                  Cancel
               </button>
               <button 
                  (click)="save()" 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
           ...
           <div class="lg:col-span-4 space-y-5">
             <div hlmCard class="border-none shadow-sm rounded-2xl bg-background overflow-hidden sticky top-24">
                <div class="aspect-[4/3] bg-muted relative group overflow-hidden">
                   <img [src]="localData().image || stockImage" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                   ...
                   <div class="absolute bottom-4 left-5 right-5">
                      ...
                      <p class="text-primary font-black text-sm">{{ (localData().price || 0) | currency }}</p>
                   </div>
                </div>
                <div hlmCardContent class="p-5 space-y-3">
...
export class AdminProductEditComponent implements OnInit, CanComponentDeactivate {
   ...
   save() {
      ...
      if (this.isEditMode()) {
         this.productStore.updateProduct(finalProduct);
      } else {
         this.productStore.addProduct(finalProduct);
      }
